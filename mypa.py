...
parser = argparse.ArgumentParser(description='MyPA Voice Control Center')
...
class Jasper(object):
    ...
    def __init__(self):
        ...
        if not os.access(jasperpath.CONFIG_PATH, os.W_OK):
            self._logger.critical("Config dir %s is not writable. MyPA " +
    ...
    def run(self):
        ...
        conversation = Conversation("MyPA", self.mic, self.config)
...
if __name__ == "__main__":
    ...
    if not args.no_network_check and not diagnose.check_network_connection():
        logger.warning("Network not connected. This may prevent MyPA from " +
                       "running properly.")
